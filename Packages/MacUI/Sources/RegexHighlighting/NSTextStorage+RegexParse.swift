//
//  NSTextStorage+RegexParse.swift
//  RegexHighlighting
//
//  CotEditor
//  https://coteditor.com
...
public extension NSTextStorage {
    ...
    ///   - enabled: If `true`, parse and highlight the text, otherwise just remove the current highlight.
    /// - Returns: Whether the content is valid as a regular expression pattern.
    @discardableResult
    final func highlightAsRegularExpressionPattern(mode: RegexParseMode, theme: RegexTheme<NSColor>, enabled: Bool = true) -> Bool {
        ...
        self.removeAttribute(.foregroundColor, range: NSRange(..<self.length))
