...
struct FolderFindTests {
    ...
    @Test func fileSizeRuleFiltersCandidates() async throws {
        ...
        let fileScope = FileScope(rules: [
            .fileSize(FileScope.FileSizeRule(comparison: .isLessThan, value: 10, unit: .bytes)),
        ])
        let summary = try await FolderFind.find(in: rootURL, query: Self.query("needle"), options: .init(fileScope: fileScope))
    ...
    @Test func fileSizeRuleComparisons() throws {
        
        let rootURL = try Self.makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: rootURL) }
        
        let fileURL = rootURL.appending(path: "a.txt")
        try Data("needle".utf8).write(to: fileURL)  // 6 bytes
        let candidate = try FolderFind.Candidate(at: fileURL)
        
        #expect(try FileScope(rules: [
            .fileSize(FileScope.FileSizeRule(comparison: .isEqualTo, value: 6, unit: .bytes)),
        ]).contains(candidate, relativeTo: rootURL))
        #expect(try FileScope(rules: [
            .fileSize(FileScope.FileSizeRule(comparison: .isLessThan, value: 0.007, unit: .kilobytes)),
        ]).contains(candidate, relativeTo: rootURL))
        #expect(try FileScope(rules: [
            .fileSize(FileScope.FileSizeRule(comparison: .isGreaterThan, value: 5, unit: .bytes)),
        ]).contains(candidate, relativeTo: rootURL))
        #expect(try !FileScope(rules: [
            .fileSize(FileScope.FileSizeRule(comparison: .isGreaterThan, value: 1, unit: .megabytes)),
        ]).contains(candidate, relativeTo: rootURL))
    }
    
    
    @Test func fileSizeRuleConvertsUnitsToBytes() {
        
        #expect(FileScope.FileSizeRule(comparison: .isEqualTo, value: 6, unit: .bytes).byteCount == 6)
        #expect(FileScope.FileSizeRule(comparison: .isEqualTo, value: 1.5, unit: .kilobytes).byteCount == 1_500)
        #expect(FileScope.FileSizeRule(comparison: .isEqualTo, value: 0.5, unit: .megabytes).byteCount == 500_000)
    }
    
    
    @Test func fileSizeRuleRejectsInvalidValue() {
        
        #expect(throws: FileScope.Error.invalidSizeValue) {
            try FileScope.FileSizeRule(comparison: .isEqualTo, value: -1, unit: .bytes).validate()
        }
        #expect(throws: FileScope.Error.invalidSizeValue) {
            _ = try FileScope.Matcher(FileScope(rules: [
                .fileSize(FileScope.FileSizeRule(comparison: .isEqualTo, value: -1, unit: .bytes)),
            ]))
        }
        
        // a “less than zero” rule can never match
        #expect(throws: FileScope.Error.invalidSizeValue) {
            try FileScope.FileSizeRule(comparison: .isLessThan, value: 0, unit: .bytes).validate()
        }
        #expect(throws: Never.self) {
            try FileScope.FileSizeRule(comparison: .isEqualTo, value: 0, unit: .bytes).validate()
        }
        #expect(throws: Never.self) {
            try FileScope.FileSizeRule(comparison: .isGreaterThan, value: 0, unit: .bytes).validate()
        }
    }
    
    
    @Test func fileSizeRuleRejectsUnreachableValue() {
        
        let maximum = 1_000_000_000  // 1 GB
        
        // rules matching only files beyond the searchable maximum are invalid
        #expect(throws: FileScope.Error.unreachableSizeValue(maximumFileSize: maximum)) {
            try FileScope.FileSizeRule(comparison: .isGreaterThan, value: 1_000, unit: .megabytes).validate(maximumFileSize: maximum)
        }
        #expect(throws: FileScope.Error.unreachableSizeValue(maximumFileSize: maximum)) {
            try FileScope.FileSizeRule(comparison: .isEqualTo, value: 1_001, unit: .megabytes).validate(maximumFileSize: maximum)
        }
        
        #expect(throws: Never.self) {
            try FileScope.FileSizeRule(comparison: .isGreaterThan, value: 999, unit: .megabytes).validate(maximumFileSize: maximum)
        }
        #expect(throws: Never.self) {
            try FileScope.FileSizeRule(comparison: .isEqualTo, value: 1_000, unit: .megabytes).validate(maximumFileSize: maximum)
        }
        #expect(throws: Never.self) {
            try FileScope.FileSizeRule(comparison: .isLessThan, value: 2_000, unit: .megabytes).validate(maximumFileSize: maximum)
        }
        
        // without a maximum, the reachability is not checked
        #expect(throws: Never.self) {
            try FileScope.FileSizeRule(comparison: .isGreaterThan, value: 1_000, unit: .megabytes).validate()
        }
        
        // the scope-level validation passes the maximum through
        #expect(throws: FileScope.Error.unreachableSizeValue(maximumFileSize: maximum)) {
            try FileScope(rules: [
                .fileSize(FileScope.FileSizeRule(comparison: .isGreaterThan, value: 1_000, unit: .megabytes)),
            ]).validate(maximumFileSize: maximum)
        }
    }
    ...
    @Test func fileScopeRoundTripsThroughCodable() throws {
        ...
        let fileScope = FileScope(rules: [
            .text(FileScope.TextRule(target: .filename, comparison: .contains, value: "readme")),
            .fileSize(FileScope.FileSizeRule(comparison: .isLessThan, value: 1.5, unit: .megabytes)),
        ])
