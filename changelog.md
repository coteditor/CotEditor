...
- 「編集」に「行を選択」コマンドを追加
- ステータスバーおよび情報ドロワーに新規に「文字数」を追加し、過去の「文字数」は「文字長」という名前に改名
    - 「文字長」は愚直にUTF-16 (= OS Xでの文字列内部表現) での length を表示するのに対して「文字数」は表示される文字単位でカウントを行なう（例えば、絵文字などのサロゲートペアは文字数:1, 文字長:2となる）
