...
extension String {
    ...
    var precomposedStringWithCompatibilityMappingWithCasefold: String {
        ...
        guard U_SUCCESS(error) else {
            print(String(format: "unorm2_getInstance failed - %s", u_errorName(error)))
            return self
        }
        ...
        guard U_SUCCESS(error) else {
            print(String(format: "u_strFromUTF8 failed - %s", u_errorName(error)))
            return self
        }
        ...
        unorm2_normalize(normalizer, utf16Source, -1, &utf16Destination, length,
         &error)
        guard U_SUCCESS(error) else {
            print(String(format: "unorm2_normalize failed - %s", u_errorName(error)))
            return self
        }
        ...
        guard U_SUCCESS(error) else {
            print(String(format: "u_strToUTF8 failed - %s", u_errorName(error)))
            return self
        }
    ...
    var precomposedStringWithHFSPlusMapping: String {
        ...
        regex.enumerateMatches(in: self, options: [], range: self.nsRange) { (match: TextCheckingResult?, flags, stop) in
        ...
        guard composed else { return self }
